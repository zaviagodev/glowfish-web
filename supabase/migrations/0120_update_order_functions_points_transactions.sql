...
DROP FUNCTION IF EXISTS create_order(text, uuid, text, numeric, numeric, numeric, numeric, numeric, text, text[], jsonb, uuid, uuid, jsonb);
...
BEGIN
  ...
  IF v_order.status IN ('cancelled', 'completed', 'delivered') THEN
  ...
  IF v_order.loyalty_points_used > 0 THEN
    -- Delete the points transaction entry for the order
  ...
  -- Process each order item
  FOR v_order_item IN 
    SELECT oi.*, p.track_quantity
    FROM order_items oi
    JOIN product_variants pv ON pv.id = oi.variant_id
    JOIN products p ON p.id = pv.product_id
    WHERE oi.order_id = p_order_id
  LOOP
    -- Return inventory if tracking is enabled
    IF v_order_item.track_quantity THEN
      UPDATE product_variants
      SET 
