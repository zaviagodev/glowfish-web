...
-- Recreate with applied_coupons support
...
-- Add helpful comment
COMMENT ON FUNCTION create_order IS 'Creates an order with items and handles inventory updates in a single transaction';

-- Add applied_coupons column if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 
    ...
    AND column_name = 'applied_coupons'
  ) THEN
