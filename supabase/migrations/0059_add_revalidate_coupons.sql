-- Create function to revalidate and recalculate applied coupons
...
BEGIN
  -- Always start with a clean slate for totals
  NEW.discount := 0;
  ...
  IF NEW.applied_coupons IS NULL OR jsonb_array_length(NEW.applied_coupons) = 0 THEN
    ...
    NEW.total := NEW.subtotal + NEW.shipping + NEW.tax;
    RETURN NEW;
  END IF;
  ...
  NEW.total := NEW.subtotal - v_total_discount + NEW.shipping + NEW.tax;
...
-- Add helpful comment
COMMENT ON FUNCTION revalidate_applied_coupons IS 'Revalidates and recalculates applied coupons when an order is created or updated'; 