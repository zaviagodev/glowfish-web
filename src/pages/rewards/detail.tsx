...
import { BarcodeIcon, CouponCodeIcon, DownloadIcon, GiftIcon, QRCodeIcon } from "@/components/icons/MainIcons";
...
const RewardDetail = () => {
  ...
  const tabsClassName = "w-full rounded-none flex py-4 gap-2 items-center text-[#979797] text-xs font-semibold box-border border-b border-[#282828] !bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-white data-[state=active]:text-white"
  const [codeType, setCodeType] = useState("barcode")

  return (
    <>
      <Header className="bg-transparent border-0" />
      {myRewardsList.filter(data => data.id == id).map(data => (
        <>
          <img src={data.image} className="w-full z-100 h-[316px] object-cover"/>
          <section className="p-5 bg-background relative -top-10 backdrop-blur-sm rounded-[14px] flex flex-col gap-7 mb-[120px]">
            ...
            <div className="grid grid-cols-2">
              <div className="flex flex-col gap-1 pr-7 border-r border-r-[#FFFFFF1A]">
                ...
                <h2 className="text-mainorange text-2xl font-semibold">{data.points} {data.points === 1 ? t("point") : t("points")}</h2>
              </div>
          ...
          <footer className="btn-footer flex flex-col gap-7">
            <Sheet open={isUsingCoupon} onOpenChange={setIsUsingCoupon}>
              ...
              <SheetContent className="h-3/4 border-0 outline-none bg-background rounded-t-2xl p-5 flex flex-col justify-between" side="bottom">
